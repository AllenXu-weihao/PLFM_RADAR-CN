...
// ============================================================================
// 数字下变频模块（ddc_400m_enhanced.v）
// ============================================================================
// 功能概述：
// 本模块在 400MHz 时钟域实现完整的数字下变频（DDC）。
// 
// 架构（787 行代码）：
// 1. ADC 符号转换（8位无符号 → 18位有符号）
// 2. NCO（数字控制振荡器，6 级流水，120MHz IF sin/cos 生成）
// 3. 混频器（2 个 DSP48E1 实例，I 和 Q 通道全流水线）
// 4. CIC 5 级 4x 抽取滤波器（400MHz → 100MHz）
// 5. FIR 32 抽头低通滤波器（9 级流水）
// 6. CDC（跨时钟域处理，Gray-code 同步）
// 
// 关键特性：
// - 在 400MHz 高时钟频率下工作（时序收敛关键）
// - 使用 DSP48E1 原语级优化（PCOUT→PCIN 级联、全流水线）
// - NCO 使用 LUTRAM（distributed RAM）满足 400MHz 时序
// - 复位同步器（2 级异步断言+同步释放，解决 400MHz 复位去断言时序违例）
// 
// 时钟域：clk_400m (400MHz) → clk_100m (100MHz)
// 输入：adc_data (8bit, 400MHz)
// 输出：baseband_i/q (18bit, 100MHz)
// ============================================================================
